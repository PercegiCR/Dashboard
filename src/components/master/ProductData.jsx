...
const ProductData = () => {
  ...
  const [formData, setFormData] = useState({ id: null, code: '', name: '', category: '', price: 0 });

  const handleOpenModal = (item = null) => {
    if (item) {
      setFormData(item);
    } else {
      setFormData({ id: null, code: '', name: '', category: '', price: 0 });
    }
    setIsModalOpen(true);
  };
            ...
            {products.map(item => (
              <tr key={item.id} className="hover:bg-gray-50 transition-colors border-b border-gray-100 last:border-0 text-gray-700">
                ...
                <td className="p-4 flex justify-end gap-3">
                  ...
                  <button onClick={() => deleteProduct(item.id)} className="text-red-500 hover:text-red-700 transition-colors p-1">
                    <Trash2 size={18} />
                  </button>
                </td>
              </tr>
